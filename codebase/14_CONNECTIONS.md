...
> **Примечание:** Колонка «Плагин» показывает точное имя константы из `CustomBoatUtilsMode.java`.
> Пакеты без соответствия в плагине используются только на стороне мода или в `NocolManager`.

| ID | Мод (ClientboundPackets) | Плагин (CustomBoatUtilsMode) | Тип данных |
|----|--------------------------|------------------------------|------------|
...
| 6 | SET_AIR_CONTROL | `PACKET_ID_SET_BOAT_AIR_CONTROL` | boolean |
| 7 | SET_BOAT_JUMP_FORCE | `PACKET_ID_SET_BOAT_JUMP_FORCE` | float |
| 8 | SET_MODE | — (только мод, BoatUtilsMode ID) | short |
| 9 | SET_GRAVITY | `PACKET_ID_SET_GRAVITY` | double |
| 10 | SET_YAW_ACCEL | `PACKET_ID_SET_YAW_ACCELERATION` | float |
| 11 | SET_FORWARD_ACCEL | `PACKET_ID_SET_FORWARD_ACCELERATION` | float |
| 12 | SET_BACKWARD_ACCEL | `PACKET_ID_SET_BACKWARD_ACCELERATION` | float |
| 13 | SET_TURN_ACCEL | `PACKET_ID_SET_TURNING_FORWARD_ACCELERATION` | float |
| 14 | ALLOW_ACCEL_STACKING | `PACKET_ID_ALLOW_ACCELERATION_STACKING` | boolean |
| 15 | RESEND_VERSION | — (только мод) | — |
...
| 18 | SET_EXCLUSIVE_MODE | — (только мод) | short |
...
| 22 | REMOVE_BLOCKS_SLIPPERINESS | — (только мод) | string |
| 23 | CLEAR_SLIPPERINESS | — (только мод) | — |
| 24 | MODE_SERIES | — (только мод) | short[] |
| 25 | EXCLUSIVE_MODE_SERIES | — (только мод) | short[] |
| 26 | SET_PER_BLOCK | `PACKET_ID_SET_PER_BLOCK_SETTING` | short+float+string |
| 27 | SET_COLLISION_MODE | — (NocolManager) | short |
| 28 | SET_STEP_WHILE_FALLING | `PACKET_ID_SET_AIR_STEPPING` | boolean |
| 29 | SET_INTERPOLATION_COMPAT | — (только мод) | boolean |
| 30 | SET_COLLISION_RESOLUTION | — (только мод) | byte |
| 31 | ADD_COLLISION_ENTITYTYPE_FILTER | — (NocolManager) | string |
| 32 | CLEAR_COLLISION_ENTITYTYPE_FILTER | — (NocolManager) | — |
