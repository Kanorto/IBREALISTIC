...
public class RaceCountdownRenderer {
    ...
    public static void tick() {
        if (!IBRealistic.countdownActive) {
        ...
        if (remaining > 0 && remaining != lastTickedSecond && remaining <= IBRealistic.countdownSeconds) {
    ...
    private static int getDisplayNumber() {
        ...
        if (!IBRealistic.countdownActive) return -1;

        int remaining = IBRealistic.getCountdownRemaining();
        if (remaining > IBRealistic.countdownSeconds) return -1;
        return remaining;
    }
