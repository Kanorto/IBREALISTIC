...
public class SQLiteDatabase extends MySQLDatabase {
    ...
    private static void updateDatabase(int previousVersion) throws SQLException {
        ...
        if (previousVersion < 23) {
            Version23.updateSQLite();
        }

        if (previousVersion < 24) {
            Version24.updateSQLite();
        }

        if (previousVersion < 25) {
            Version25.updateSQLite();
        }

        if (previousVersion < 26) {
            Version26.updateSQLite();
        }

        if (previousVersion < 27) {
            Version27.updateSQLite();
        }
    }
