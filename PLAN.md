...
## ФАЗА 5a: СИСТЕМА СПАВНА ИГРОКОВ ✅
- [x] Телепортация игроков на спавн при заходе на сервер
- [x] Телепортация на спавн при респавне (смерти)
- [x] Настраиваемые координаты и мир спавна через config.yml
- [x] Хотбар-предметы при заходе:
  - [x] Слот 0: NETHER_STAR — команда /tt (треки), кулдаун 1 сек
  - [x] Слот 1: RECOVERY_COMPASS — команда /reset, кулдаун 2 сек
  - [x] Слот 8: CHERRY_BOAT — команда /b (лодка), кулдаун 2 сек
- [x] Предметы нельзя выбросить, но можно перемещать в хотбаре
- [x] Система кулдаунов для каждого предмета
- [x] Поддержка перевода через Triton (языковые ключи)

## ФАЗА 6: КАСТОМИЗАЦИЯ АВТОМОБИЛЕЙ (БУДУЩЕЕ) ⏳

### 6.1 Система пресетов компонентов
- [ ] Пресеты шин (разные характеристики сцепления, износа, температурного окна)
  - Примеры: Soft, Medium, Hard, Rain, Ice
  - Параметры: μ_peak, μ_slide, corneringStiffness, wearRate, temperatureRange
- [ ] Пресеты двигателей (мощность, крутящий момент, вес)
  - Примеры: Street, Sport, Rally, Turbo, Electric
  - Параметры: maxPower, torqueCurve, weight, reliability
- [ ] Пресеты кузовов (аэродинамика, вес, прочность)
  - Примеры: Hatchback, Sedan, SUV, Rally_Spec
  - Параметры: dragCoefficient, downforce, weight, durability

### 6.2 Система сборки автомобиля
- [ ] Игрок собирает автомобиль из пресетов (шины + двигатель + кузов)
- [ ] Каждая комбинация создаёт уникальный CustomBoatUtilsMode
- [ ] Нельзя создавать произвольные характеристики — только из готовых пресетов
- [ ] Сохранение конфигурации автомобиля игрока в БД

### 6.3 Внутриигровая экономика
- [ ] Внутриигровая валюта (заработок за завершение треков, позиции в гонках)
- [ ] Магазин пресетов (покупка шин, двигателей, кузовов)
- [ ] Система цен и балансировка
- [ ] GUI магазина и инвентаря

### 6.4 Система поломок автомобилей
- [ ] Износ шин (деградация μ со временем, зависит от поверхности и стиля)
- [ ] Перегрев двигателя (снижение мощности при длительной максимальной нагрузке)
- [ ] Повреждения от столкновений (снижение характеристик)
- [ ] Визуальные эффекты повреждений (частицы дыма, искры)
- [ ] Ремонт в сервис-зонах (восстановление характеристик)

### 6.5 Спавн кастомного автомобиля
- [ ] Команда /car spawn — спавнит автомобиль с текущей конфигурацией игрока
- [ ] Автоматическое применение CustomBoatUtilsMode на основе собранного автомобиля
- [ ] Интеграция с хотбар-предметами (замена предмета лодки на предмет автомобиля)

### 6.6 Серверная валидация скорости (Античит)
- [ ] Плагин проверяет скорость движения игрока на сервере
- [ ] Сравнение фактической скорости с максимально допустимой для текущего режима/типа машины
- [ ] Защита от модифицированного мода (клиент не может отправлять произвольную скорость)
- [ ] Логирование подозрительных движений
- [ ] Настраиваемые пороги и допуски для сетевых задержек
