...
## Исправления системы гостов
- Дефолтный режим гостов: OFF → LINE (Settings.java)
- finishTimeMs: 0L → реальное время из ts_race_results (TelemetryReceiver.java)
- PB сравнение: перезаписывает только если быстрее (TelemetryReceiver.java)
